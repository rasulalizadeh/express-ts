...
const { GetConfig } = require("express-vue/lib/utils/config.utils");
/**
...
function init(options, renderer) {
    ...
    function expressVueMiddleware(req, res, next) {
         ...
         */
        async function StreamToClient(stream) {
         ...
         */
        res.renderVue = async function(componentPath, data = {}, vueOptions = {}) {
            return new Promise((resolve, reject) => {
                res.set("Content-Type", "text/html");
                Renderer.RenderToStream(componentPath, data, vueOptions)
                    .then((stream) => {
                        StreamToClient(stream).then(() => {
                            resolve()
                        }).catch(() => {
                            reject()
                        })
